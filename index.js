...
class Client {
    ...
    async confirmEmail({
        ...
        try {
            ...
            if (debug) console.log(`[badgr-api-client] getting "${url}"`);
            const response = await axios.get(url);
            if (debug) console.log("[badgr-api-client] response:", response);
            const { data } = response;
            if (debug) console.log("[badgr-api-client] data:", data);
            result.data = data;
            result.success = true;
        } catch (error) {
            let responseURL = "";
